...
public final class DevelopmentSeed {
  ...
  public static void core(EntityManager em) {
    ...
    for (String[] row : works) {
      if (work(em, row[0]) == null) {
        em.persist(new WorkDefinition(row[0], row[1], WorkCategory.valueOf(row[2]), null, null));
      }
    }
    em.flush();
    String[][] rules = {
    ...
    for (String[] row : rules) {
      if (em.createQuery("select count(r) from DiagnosticRule r where r.code=:code", Long.class)
              .setParameter("code", row[0])
              .getSingleResult()
          == 0) {
        em.persist(new DiagnosticRule(row[0], work(em, row[1]), row[2], Integer.parseInt(row[3])));
      }
    }
  }
  ...
  public static void demo(EntityManager em) {
    ...
    for (int i = 1; i <= 2; i++) {
      ...
      VehicleVariant v =
          em.createQuery("select v from VehicleVariant v where v.code=:code", VehicleVariant.class)
      ...
      for (String workCode : codes) {
        if (i == 2 && workCode.equals("GLOW_PLUGS")) {
          continue;
        }
        WorkDefinition w = work(em, workCode);
        long count =
            em.createQuery(
                    "select count(r) from VehicleWorkRule r where r.variant.id=:v and r.work.id=:w",
                    Long.class)
        ...
        if (count == 0) {
          ...
          String note =
              "DEMO: sinteticki podatak za testiranje, nije cijena hrvatskog trzista niti preporuka"
                  + " za stvarno vozilo.";
          em.persist(
              new VehicleWorkRule(
                  v,
                  w,
                  maintenance ? 10000 : null,
                  maintenance ? 12 : null,
                  new BigDecimal(maintenance ? "50.00" : "200.00"),
                  maintenance ? note : null,
                  note));
        }
      }
    }
  ...
  private static WorkDefinition work(EntityManager em, String code) {
    return em.createQuery("select w from WorkDefinition w where w.code=:code", WorkDefinition.class)
        .setParameter("code", code)
        .getResultStream()
        .findFirst()
        .orElse(null);
