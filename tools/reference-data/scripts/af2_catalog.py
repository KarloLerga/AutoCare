...
OUT="code make model generation engine_label body_type fuel_type power_hp transmission year_from year_to".split()
...
def normalize(row: dict[str,str])->dict[str,str|int|None]:
    ...
    return dict(code=code,make=r["make"],model=r["model"],generation=r["generation"],engine_label=r["engine_label"],body_type=r["body_type"],fuel_type=r["fuel_type"],power_hp=hp,transmission=r["transmission"],year_from=start,year_to=end)
