...
public final class EstimateSelection {
    ...
    private static final Set<Set<String>> OVERLAPS = Set.of(
    ...
    public static Set<String> conflicts(Set<String> selectedCodes, String candidateCode) {
        ...
        if (selectedCodes.contains(candidateCode)) result.add(candidateCode);
        for (Set<String> pair : OVERLAPS) {
            if (!pair.contains(candidateCode)) continue;
            for (String code : pair) {
                if (!code.equals(candidateCode) && selectedCodes.contains(code)) result.add(code);
            }
        }
