...
public final class OnboardingDialog extends JDialog {
  ...
  public final JButton back = Ui.button("Natrag", false),
      next = Ui.button("Nastavi", true),
      finish = Ui.button("Zavrsi registraciju", true),
      cancel = Ui.button("Odustani", false),
      addHistory = Ui.button("Dodaj poznati servis", false),
      removeHistory = Ui.button("Ukloni odabrani servis", false);
  public final DataTable<ServiceInput> history =
      new DataTable<>(
          new String[] {"Datum", "Km", "Broj stavki", "Napomena"},
          (s, c) ->
              switch (c) {
                case 0 -> s.getDate();
                case 1 -> s.getMileage();
