...
public final class EstimateFormat {
    ...
    public static BigDecimal rounded(BigDecimal amount) {
        ...
        if (amount.signum() < 0) throw new IllegalArgumentException("Negativna procjena.");
        return amount.divide(BigDecimal.TEN, 0, RoundingMode.HALF_UP)
                     .multiply(BigDecimal.TEN);
    }

    public static String display(BigDecimal amount) {
        if (amount == null) return "Nema procjene";
