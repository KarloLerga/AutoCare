...
public final class Ui {
    ...
    public static JPanel row(Component... controls) {
        ...
        p.setOpaque(false);
        for(Component c:controls)p.add(c);
        return p;
    }
    ...
    public static JPanel card() {
        ...
        p.setBorder(new CompoundBorder(new LineBorder(new Color(0xDCE5ED)), new EmptyBorder(16, 16, 16, 16)));
        return p;
    }
    ...
    public static BigDecimal parseMoney(String text, boolean optional) {
        ...
        String t=text.strip();
        if(!t.matches("[0-9]+([.,][0-9]{1,2})?"))throw new IllegalArgumentException("Cijena: npr. 120,50; bez simbola EUR i odvajanja tisucica.");
        return Checks.money(new BigDecimal(t.replace(',', '.')), optional);
    }
    public static String money(BigDecimal value) {
        return value==null?"Nepoznato":String.format(Locale.forLanguageTag("hr-HR"), "%,.2f EUR", value);
    }
    public static String total(CostSummary c) {
        return money(c.getKnownTotal())+(c.getUnknownCount()>0?" + "+c.getUnknownCount()+" nepoznatih":"");
    }
    ...
    public static boolean confirm(Component parent, String text) {
        return JOptionPane.showConfirmDialog(parent, text, "AutoCare", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE)==JOptionPane.YES_OPTION;
    }
    ...
    public static void error(Component parent, Throwable error) {
        JOptionPane.showMessageDialog(parent, error.getMessage()==null?"Operacija nije uspjela.":error.getMessage(), "AutoCare", JOptionPane.ERROR_MESSAGE);
    }
    public static void escape(JDialog dialog, Runnable close) {
        dialog.getRootPane().registerKeyboardAction(e->close.run(), KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), JComponent.WHEN_IN_FOCUSED_WINDOW);
    ...
    private static void capture(Component c, Map<Component, Boolean> old) {
        ...
        c.setEnabled(false);
        if(c instanceof Container panel)for(Component child:panel.getComponents())capture(child, old);
    }
