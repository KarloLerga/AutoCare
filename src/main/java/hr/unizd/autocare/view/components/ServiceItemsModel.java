...
public final class ServiceItemsModel extends AbstractTableModel {
    ...
    public void add(WorkRow work) {
        for(Line line:lines)if(line.work.getId()==work.getId())throw new IllegalArgumentException("Rad je vec u servisu.");
        int row=lines.size();
    ...
    public List<ItemInput> snapshot(boolean history) {
        ...
        for(Line line:lines)out.add(new ItemInput(line.work.getId(), Ui.parseMoney(line.amount, history)));
