...
public final class VehicleImage extends JLabel {
  ...
  /**
   * Shows an approved local path, or a generated category illustration when the catalog still
   * contains the neutral fallback. The illustration is deliberately not presented as an exact
   * make/model match.
   */
  public void showVehicle(String path, String fuelType) {
    ...
    String key =
        path != null
                && path.startsWith("/images/vehicles/")
                && !path.contains("..")
                && !path.equals(FALLBACK)
            ? path
            : categoryFallback;
    ImageIcon icon = load(key);
    if (icon == null) {
      icon = load(categoryFallback);
    }
    if (icon == null) {
      icon = load(FALLBACK);
    }
    setIcon(icon);
    setText(icon == null ? "Slika nije dostupna" : "");
    setToolTipText(
        key.equals(GENERIC) || key.equals(ELECTRIC)
            ? "Generička ilustracija — nije fotografija točnog modela."
            : null);
  }
  ...
  private static synchronized ImageIcon load(String path) {
    ...
    URL url = VehicleImage.class.getResource(path);
    if (url == null) {
      return null;
    }
    try {
      ...
      int w = Math.max(1, (int) (image.getWidth() * ratio)),
          h = Math.max(1, (int) (image.getHeight() * ratio));
