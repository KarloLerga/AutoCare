...
public final class DashboardView extends JPanel {
    private final JLabel total=Ui.hint("-"), maintenance=Ui.hint("-"), problems=Ui.hint("-"), mileage=Ui.hint("-");
    public DashboardView() {
        ...
        JLabel[] values= {
            total, maintenance, problems, mileage
        };
        for(int i=0; i<4; i++) {
            JPanel card=Ui.card();
        ...
        add(Ui.hint("Procjene su informativne. Nisu racun ni dijagnoza mehanicara."), BorderLayout.SOUTH);
    }
    public void show(Dashboard d) {
        total.setText(Ui.total(d.getTotal()));
        maintenance.setText(d.getCovered()==0?"Nema pravila za ovu varijantu":d.getDue()+" dospjelo / "+d.getSoon()+" uskoro / "+d.getUnknown()+" nepoznato");
