...
public final class ServiceEditorDialog extends JDialog {
    ...
    public ServiceEditorDialog(Window owner, int km, boolean historical, List<ProblemRow> problems) {
        super(owner, historical?"Pocetna povijest - novi zapis":"Novi servis", ModalityType.APPLICATION_MODAL);
        ...
        lower.add(Ui.hint(historical?"Nepoznata stara cijena ostaje prazna; 0 znaci poznat nulti trosak.":"Cijena je stvarno placeni ukupan iznos po radu. Npr. 120,50."));
        if(!problems.isEmpty()) {
            JTable p=new JTable(new AbstractTableModel() {
    ...
    public ServiceInput input() {
        if(itemTable.isEditing()&&!itemTable.getCellEditor().stopCellEditing())throw new IllegalArgumentException("Potvrdite valjanu cijenu u tablici.");
        List<Long> ids=new ArrayList<>();
        for(int i=0; i<selected.length; i++)if(selected[i])ids.add(problems.get(i).getId());
        return new ServiceInput(requestKey, date.date(), Ui.integer(mileage), note.getText(), items.snapshot(historical), ids);
    }
