...
public final class MaintenanceView extends JPanel {
    ...
    public final DataTable<MaintenanceRow> table=new DataTable<>(new String[] {
        "Rad", "Zadnji datum", "Zadnji km", "Sljedeci datum", "Sljedeci km", "Status", "Preostalo", "Izvor intervala"
    }, (m, c)->switch(c) {
        case 0->m.getName();
    ...
    private static String remaining(MaintenanceRow row) {
        if(row.getStatus()==hr.unizd.autocare.domain.MaintenanceStatus.CONDITION_BASED)return "Prema stanju";
        if(row.getStatus()==hr.unizd.autocare.domain.MaintenanceStatus.VEHICLE_INDICATOR)return "Prema indikatoru";
        ...
        if(km==null&&days==null)return "Nepoznato";
