...
public final class CsvReader implements AutoCloseable {
    ...
    public List<String> readRow()throws IOException {
        ...
        while(true) {
            if(value.length()>1000000)throw new IOException("CSV polje je preveliko.");
            int ch=reader.read();
            if(first) {
                first=false;
                if(ch==0xFEFF)ch=reader.read();
            }
            if(ch==-1) {
                if(quoted)throw new IOException("Nedovrseno navodno polje u CSV-u.");
                if(!seen&&fields.isEmpty())return null;
            ...
            if(c=='"') {
                if(value.length()!=0||closed)throw new IOException("Navodnik na nevaljanom mjestu u CSV-u.");
                quoted=true;
                continue;
            }
            if(closed)throw new IOException("Tekst nakon zatvorenog CSV navodnika.");
            value.append(c);
        }
    }
