...
public final class VehicleService {
    ...
    public List<VehicleRow> list(long owner) {
        return tx.read(r-> {
            ...
            for(Vehicle v:r.vehicles().list(owner))rows.add(Mapping.vehicle(v, active));
    ...
    public VehicleRow active(long owner) {
        return tx.read(r-> {
            User u=r.users().require(owner);
            if(u.getActiveVehicle()==null)throw AppException.conflict("Racun nema aktivno vozilo. Provjerite integritet baze.");
            return Mapping.vehicle(r.vehicles().requireOwned(owner, u.getActiveVehicle().getId()), u.getActiveVehicle().getId());
        });
    }
    public long add(long owner, VehicleInput input) {
        ...
        return tx.write(r-> {
            ...
            Vehicle v=new Vehicle(u, r.catalog().variant(input.getVariantId()), input.getYear(), input.getMileage());
            r.vehicles().add(v);
            return v.getId();
        });
    }
    public void update(long owner, long vehicle, long expected, VehicleInput input) {
        ...
        tx.write(r-> {
            ...
            boolean identityChanged=!v.getVariant().getId().equals(input.getVariantId())||v.getYear()!=input.getYear();
            if(identityChanged&&r.vehicles().hasHistory(vehicle))throw AppException.conflict("Identitet vozila s povijescu nije moguce promijeniti.");
            if(identityChanged)v.changeIdentity(r.catalog().variant(input.getVariantId()), input.getYear());
    ...
    public void delete(long owner, long vehicle, Long replacement) {
        tx.write(r-> {
            ...
            if(all.size()<=1)throw AppException.conflict("Posljednje vozilo nije moguce obrisati.");
            Vehicle v=r.vehicles().requireOwned(owner, vehicle);
            if(u.getActiveVehicle()!=null&&u.getActiveVehicle().getId().equals(vehicle)) {
                if(replacement==null||replacement.equals(vehicle))throw AppException.validation("Odaberite drugo vlastito aktivno vozilo.");
    ...
    private void validateYear(VehicleInput input) {
        if(input.getYear()<1886||input.getYear()>LocalDate.now(clock).getYear())throw AppException.validation("Nevaljana godina proizvodnje.");
