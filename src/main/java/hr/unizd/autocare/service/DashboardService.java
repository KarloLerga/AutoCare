    ...
    return tx.read(
        r -> {
          ...
          for (MaintenanceRow m : rows) {
            if (m.getStatus() == MaintenanceStatus.DUE) {
              due++;
            } else if (m.getStatus() == MaintenanceStatus.SOON) {
              soon++;
            } else if (m.getStatus() == MaintenanceStatus.UNKNOWN_HISTORY
                || m.getStatus() == MaintenanceStatus.UNKNOWN_INTERVAL) {
              unknown++;
            }
          }
          return new Dashboard(
              Mapping.vehicle(v, vehicle),
              r.services().total(owner, vehicle),
              r.problems().openCount(owner, vehicle),
              due,
              soon,
              unknown,
              rows.size());
