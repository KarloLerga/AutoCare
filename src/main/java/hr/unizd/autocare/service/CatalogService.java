...
public final class CatalogService {
    ...
    public List<VariantRow> variants(int year, String make, String model, String search) {
        return tx.read(r-> {
            ...
            for(VehicleVariant v:r.catalog().variants(year, make, model, search))out.add(Mapping.variant(v));
    ...
    public List<WorkRow> onboardingWorks(long variant, WorkCategory category) {
        return tx.read(r-> { r.catalog().variant(variant); return workRows(r,variant,category); });
    }
    static List<WorkRow> workRows(Repositories r, long variant, WorkCategory category) {
        Map<Long, VehicleWorkRule> rules=new HashMap<>();
        for(VehicleWorkRule rule:r.catalog().rules(variant))rules.put(rule.getWork().getId(), rule);
        List<WorkRow> out=new ArrayList<>();
        for(WorkDefinition w:r.catalog().works(category)) {
            VehicleWorkRule rule=rules.get(w.getId());
            if(rule==null && !w.getCode().startsWith("OTHER_"))continue;
            boolean specific=rule!=null&&rule.getEstimatedPrice()!=null;
            out.add(new WorkRow(w.getId(), w.getCode(), w.getName(), w.getCategory(), specific?rule.getEstimatedPrice():null, rule!=null?rule.getEstimateNote():w.getEstimateNote()));
        }
        return List.copyOf(out);
    }
}
