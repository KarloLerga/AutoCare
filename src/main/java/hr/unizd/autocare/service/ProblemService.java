...
public final class ProblemService {
    ...
    public List<ProblemRow> list(long owner, long vehicle, ProblemStatus status) {
        return tx.read(r-> {
            ...
            for(Problem p:r.problems().list(owner, vehicle, status))rows.add(Mapping.problem(p));
    ...
    private Analysis analysis(Repositories r, Vehicle v, String description) {
        ...
        for(VehicleWorkRule rule:r.catalog().rules(v.getVariant().getId()))if(rule.getWork().getCategory()==WorkCategory.REPAIR)supported.add(rule.getWork().getId());
        Map<Long, WorkRow> prices=new HashMap<>();
        for(WorkRow w:CatalogService.workRows(r, v.getVariant().getId(), WorkCategory.REPAIR))prices.put(w.getId(), w);
        ...
        for(DiagnosticRule rule:r.catalog().diagnosticRules()) {
            ...
            if(w!=null && supported.contains(w.getId()))rules.add(new RuleData(w.getId(), w.getName(), rule.getPhrase(), rule.getWeight(), w.getPrice(), w.getPriceNote()));
        }
    ...
    public long save(long owner, long vehicle, String requestKey, Analysis preview) {
        if(preview==null)throw AppException.validation("Prvo analizirajte opis.");
        return tx.write(r-> {
            r.users().lock(owner);
            Optional<Problem> old=r.problems().byRequest(owner, requestKey);
            if(old.isPresent()) {
                if(!old.get().getVehicle().getId().equals(vehicle))throw AppException.conflict("Zahtjev pripada drugom vozilu.");
                if(!old.get().getDescription().equals(Checks.text(preview.getDescription(), 2000, "Opis simptoma")))throw AppException.conflict("Isti zahtjev vec ima spremljen drugi opis.");
                return old.get().getId();
            }
            Vehicle v=r.vehicles().requireOwned(owner, vehicle);
            Analysis fresh=analysis(r, v, Checks.text(preview.getDescription(), 2000, "Opis simptoma"));
            if(!same(preview, fresh))throw AppException.conflict("Pravila ili cijene su se promijenili. Ponovite analizu.");
            DiagnosticResult top=fresh.getResults().isEmpty()?null:fresh.getResults().get(0);
            Problem p=new Problem(v, requestKey, fresh.getDescription(), LocalDateTime.now(clock), top==null?null:r.catalog().work(top.getCandidateId()), top==null?null:top.getScore(), top==null?null:top.getPrice(), top==null?null:top.getPriceNote());
            r.problems().add(p);
            return p.getId();
        });
    }
    private boolean same(Analysis a, Analysis b) {
        if(a.getResults().size()!=b.getResults().size())return false;
        for(int i=0; i<a.getResults().size(); i++) {
            DiagnosticResult x=a.getResults().get(i), y=b.getResults().get(i);
            if(x.getCandidateId()!=y.getCandidateId()||x.getScore().compareTo(y.getScore())!=0||!Objects.equals(x.getPrice(), y.getPrice())||!Objects.equals(x.getPriceNote(), y.getPriceNote()))return false;
        }
        return true;
    }
