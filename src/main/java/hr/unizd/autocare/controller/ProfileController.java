...
public final class ProfileController {
    ...
    public ProfileController(MainFrame frame, AuthService service, Session session, AppEvents events, Runnable logout) {
        ...
        frame.profile.logout.addActionListener(e-> {
            if(canLeave())logout.run();
        });
    }
    public void load() {
        long owner=session.owner();
        tasks.read(frame.profile, ()->service.account(owner), account-> {
            original=account;
            discard();
        });
    }
    public boolean canLeave() {
        ...
        char[] current=frame.profile.current.getPassword(), next=frame.profile.next.getPassword(), repeat=frame.profile.repeat.getPassword();
        boolean dirty=!original.getName().equals(frame.profile.name.getText())||!original.getEmail().equals(frame.profile.email.getText())||current.length>0||next.length>0||repeat.length>0;
        Arrays.fill(current, '\0');
        Arrays.fill(next, '\0');
        Arrays.fill(repeat, '\0');
        if(!dirty)return true;
        if(!Ui.confirm(frame, "Odbaciti nespremljene promjene profila?"))return false;
        discard();
        return true;
    }
    public void clear() {
