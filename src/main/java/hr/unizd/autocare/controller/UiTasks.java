...
public final class UiTasks {
    ...
    public <T>void run(Component parent, boolean write, Callable<T> work, Consumer<T> success, Consumer<Throwable> failure) {
        ...
        if(write)session.setWriting(true);
        ...
        new SwingWorker<T, Void>() {
            ...
            @Override protected void done() {
                ...
                try {
                    ...
                    if(epoch==session.epoch()&&ticket==sequence&&parent.isDisplayable())failure.accept(cause);
                    return;
                }
                catch(CancellationException ex) {
                    return;
                }
                if(epoch!=session.epoch()||ticket!=sequence||!parent.isDisplayable())return;
                try {
                    success.accept(result);
                }
                catch(RuntimeException ex) {
                    LOG.log(Level.SEVERE, write?"Write je potvrden; naknadni GUI callback nije uspio.":"GUI callback nije uspio.", ex);
                    Ui.info(parent, write?"Promjena je spremljena, ali prikaz nije osvjezen. Otvorite ekran ponovno.":"Podaci su ucitani, ali prikaz nije uspio. Osvjezite ekran.");
                }
            }
        }
        .execute();
