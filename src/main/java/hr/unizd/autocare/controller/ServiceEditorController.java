...
public final class ServiceEditorController {
    public ServiceEditorController(ServiceEditorDialog view, List<WorkRow> works, Session session, Consumer<ServiceInput> submit) {
        ...
        Runnable cancel=()-> {
            ...
            if(Ui.confirm(view, "Odbaciti nespremljene podatke ovog servisa?"))view.dispose();
        };
    ...
    private void add(ServiceEditorDialog view, List<WorkRow> all, WorkCategory category) {
        try {
            ...
            for(WorkRow w:all)if(w.getCategory()==category)choices.add(w);
            WorkRow selected=WorkPicker.choose(view, choices);
            if(selected!=null)view.items.add(selected);
        }
        catch(RuntimeException ex) {
            Ui.error(view, ex);
        }
