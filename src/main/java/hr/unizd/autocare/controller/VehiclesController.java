...
public final class VehiclesController {
    ...
    public VehiclesController(MainFrame frame, VehicleService service, CatalogService catalog, Session session, AppEvents events) {
        ...
        frame.vehicles.edit.addActionListener(e-> {
            VehicleRow row=selected();
            if(row!=null)edit(row);
        });
        frame.vehicles.activate.addActionListener(e-> {
            VehicleRow row=selected();
            if(row!=null)tasks.write(frame, ()-> {
    ...
    public void load() {
        ...
        tasks.read(frame.vehicles, ()->service.list(owner), rows->frame.vehicles.table.setRows(rows));
    }
    private VehicleRow selected() {
        VehicleRow row=frame.vehicles.table.selected();
        if(row==null)Ui.info(frame, "Odaberite vozilo.");
        return row;
    }
    private void edit(VehicleRow row) {
        if(row==null)showEditor(null, true);
        else {
            long owner=session.owner();
            tasks.read(frame, ()->service.identityEditable(owner, row.getId()), editable->showEditor(row, editable));
        }
    }
    private void showEditor(VehicleRow row, boolean identityEditable) {
        JDialog dialog=new JDialog(frame, row==null?"Dodaj vozilo":"Uredi vozilo", Dialog.ModalityType.APPLICATION_MODAL);
        ...
        save.addActionListener(e-> {
            try {
                ...
                editorTask.run(dialog, true, ()-> {
                    if(row==null)service.add(owner, input);
                    else service.update(owner, row.getId(), row.getVersion(), input);
                    return true;
                }, ok-> {
                    dialog.dispose();
                    ...
                    if(UiTasks.uncertain(error)) {
                        ...
                        Ui.info(dialog, "Ne ponavljajte dodavanje naslijepo. Zatvorite obrazac i osvjezite Vozila kako biste provjerili stvarno stanje.");
                    }
                });
            }
        ...
        Runnable close=()-> {
            if(!session.isWriting()&&Ui.confirm(dialog, "Odbaciti nespremljene promjene vozila?"))dialog.dispose();
        };
        cancel.addActionListener(e->close.run());
        Ui.escape(dialog, close);
        if(row!=null)form.existing(row, identityEditable);
        else SwingUtilities.invokeLater(picker::loadMakes);
    ...
    private void delete() {
        ...
        if(row.getActive()) {
            ...
            for(VehicleRow other:rows)if(other.getId()!=row.getId())choices.add(other);
            String[] labels=new String[choices.size()];
            for(int i=0; i<labels.length; i++) {
                VehicleRow v=choices.get(i);
                labels[i]=v.getVariant().getMake()+" "+v.getVariant().getModel()+" ("+v.getYear()+", #"+v.getId()+")";
            }
            String answer=(String)JOptionPane.showInputDialog(frame, "Odaberite novo aktivno vozilo:", "Brisanje aktivnog vozila", JOptionPane.QUESTION_MESSAGE, null, labels, labels[0]);
            if(answer==null)return;
            replacement=choices.get(Arrays.asList(labels).indexOf(answer)).getId();
        }
        if(!Ui.confirm(frame, "Trajno obrisati vozilo #"+row.getId()+", njegove servise i probleme?"))return;
