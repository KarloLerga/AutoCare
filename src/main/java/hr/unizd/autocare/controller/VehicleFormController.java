...
public final class VehicleFormController {
    ...
    public VehicleFormController(VehicleForm view, CatalogService service, Session session) {
        ...
        view.year.addChangeListener(e-> {
            if(!view.updating)loadMakes();
        });
        view.make.addActionListener(e-> {
            if(!view.updating)loadModels();
        });
        view.model.addActionListener(e-> {
            if(!view.updating)loadVariants();
        });
    ...
    public void loadMakes() {
        try {
            ...
            tasks.read(view, ()->service.makes(year), values-> {
                view.updating=true;
                view.make.setModel(new DefaultComboBoxModel<>(values.toArray(String[]::new)));
                view.make.setSelectedIndex(-1);
                view.updating=false;
                view.state.setText(values.isEmpty()?"Nema kataloga za ovu godinu. Provjerite seed.":"Odaberite marku.");
            });
        }
    ...
    private void loadModels() {
        ...
        if(make==null)return;
        int year=Ui.integer(view.year);
        tasks.read(view, ()->service.models(year, make), values-> {
            view.updating=true;
    ...
    private void loadVariants() {
        String make=(String)view.make.getSelectedItem(), model=(String)view.model.getSelectedItem(), query=view.search.getText();
        view.variants.setRows(List.of());
        tasks.invalidate();
        if(make==null||model==null)return;
        int year=Ui.integer(view.year);
        view.state.setText("Ucitavanje varijanti...");
        tasks.read(view, ()->service.variants(year, make, model, query), values-> {
            boolean more=values.size()>200;
            view.variants.setRows(more?values.subList(0, 200):values);
            view.state.setText(more?"Prikazano prvih 200. Suzite trazenje motorom ili generacijom.":values.isEmpty()?"Nema rezultata. Promijenite filtre.":"Odaberite redak; nepoznat zavrsetak raspona nije dokaz da se model jos proizvodi.");
        });
    }
}
