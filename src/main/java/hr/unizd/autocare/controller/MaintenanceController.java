...
public final class MaintenanceController {
          ...
          for (int row : selected) {
            MaintenanceRow m =
                frame
                    ...
                    .table
                    .rows()
                    .get(frame.maintenance.table.table().convertRowIndexToModel(row));
            if (!EstimateSelection.conflicts(selectedCodes, m.getWorkCode()).isEmpty()) {
              Ui.info(
                  frame,
                  "Odabrani paket vec sadrzi dio drugog zahvata. Uklonite preklopljenu stavku.");
              return;
            }
            ...
            notes
                ...
                .append(m.getName())
                .append(": ")
                .append(EstimateFormat.display(m.getPrice()))
                .append(" / ")
                .append(Objects.toString(m.getPriceNote(), "Nema izvora cijene"));
          }
          Ui.info(
              frame,
              "Informativna procjena: "
                  + EstimateFormat.display(CostSummary.of(prices).getKnownTotal())
                  + (CostSummary.of(prices).getUnknownCount() > 0
                      ? " + stavke bez procjene (zbroj nije potpun)"
                      : "")
                  + "\nNista nije spremljeno i intervali nisu promijenjeni."
                  + notes);
        });
  }
  ...
  public void load() {
    ...
    tasks.read(
        frame.maintenance,
        () -> service.list(owner, vehicle),
        rows -> {
          frame.maintenance.table.setRows(rows);
          frame.maintenance.coverage.setText(
              rows.isEmpty()
                  ? "Katalog nema pravila za varijantu. To ne znaci da odrzavanje nije potrebno."
                  : "Broj pokrivenih radova: "
                      + rows.size()
                      + ". Nepoznat rok nije preporuka. Procjene nisu ponude servisa.");
        });
  }
}
