...
import hr.unizd.autocare.strategy.*;
import hr.unizd.autocare.service.PasswordHasher;
import hr.unizd.autocare.tools.CsvReader;
...
public final class CoreChecks {
    ...
    public static void maintenance() {
        ...
        eq(MaintenanceStatus.DUE, c.calculate(null, 1, LocalDate.of(2024, 1, 31), null, 0, LocalDate.of(2024, 2, 29)));
    ...
    public static void money() {
        eq(new BigDecimal("90.00"), CostSummary.of(Arrays.asList(new BigDecimal("90.00"), null, BigDecimal.ZERO)).getKnownTotal());
    ...
    public static void diagnostics() {
        ...
        List<RuleData> rules=List.of(new RuleData(1, "A", "slabo hladi", 3, null, null), new RuleData(1, "A", "zvizdi", 1, null, null), new RuleData(2, "B", "hladi", 1, null, null));
    ...
    public static void entities() {
        User user=new User("Test", "test@example.com", "not-a-real-hash");
        VehicleVariant variant=new VehicleVariant("test", "DEMO", "A", "G", "D", 2010, 2026, "Diesel");
        ...
        WorkDefinition work=new WorkDefinition("TEST", "Testni rad", WorkCategory.MAINTENANCE, null, null);
        ServiceRecord record=new ServiceRecord(v, UUID.randomUUID().toString(), LocalDate.now(), 1000, null);
    ...
    public static void csv()throws IOException {
        try(CsvReader reader=new CsvReader(new StringReader("\ufeffa,b\r\n\"x,y\",\"one\nline\"\r\n\"a\"\"b\",c"))) {
            eq(List.of("a", "b"), reader.readRow());
            eq(List.of("x,y", "one\nline"), reader.readRow());
            eq(List.of("a\"b", "c"), reader.readRow());
            eq(null, reader.readRow());
        }
        checks++;
        try(CsvReader reader=new CsvReader(new StringReader("\"not closed"))) {
            reader.readRow();
            throw new AssertionError("CSV quote not rejected");
        }
