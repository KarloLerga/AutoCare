...
def main() -> None:
    ...
    a("    EXEC sys.sp_executesql N'ALTER TABLE dbo.vehicle_variant ADD price_class nvarchar(32) NULL;';")
    a("  EXEC sys.sp_executesql N'UPDATE dbo.vehicle_variant SET price_class=N''STANDARD'';';")
    a("")
    ...
    a("  EXEC sys.sp_executesql N'UPDATE variant SET price_class=classes.price_class FROM dbo.vehicle_variant variant JOIN #VariantClass classes ON classes.code=variant.code;';")
    a("  DECLARE @PriceClassCount bigint=0;")
    price_class_checks = [
        ("ECONOMY", 460, 51203, "Ne odgovara broj ECONOMY vozila."),
        ("STANDARD", 18777, 51204, "Ne odgovara broj STANDARD vozila."),
        ("PREMIUM", 9645, 51205, "Ne odgovara broj PREMIUM vozila."),
        ("PERFORMANCE", 783, 51206, "Ne odgovara broj PERFORMANCE vozila."),
        ("EXOTIC", 701, 51207, "Ne odgovara broj EXOTIC vozila."),
    ]
    for price_class, expected, error_number, message in price_class_checks:
        a("  EXEC sys.sp_executesql N'SELECT @Count=COUNT_BIG(*) FROM dbo.vehicle_variant WHERE price_class=@PriceClass;', N'@PriceClass nvarchar(32), @Count bigint OUTPUT', @PriceClass=" + n(price_class) + ", @Count=@PriceClassCount OUTPUT;")
        a(f"  IF @PriceClassCount <> {expected}")
        a(f"    THROW {error_number}, {n(message)}, 1;")
    a("  EXEC sys.sp_executesql N'ALTER TABLE dbo.vehicle_variant ALTER COLUMN price_class nvarchar(32) NOT NULL;';")
    a("")
    a("  IF COL_LENGTH(N'dbo.work_definition',N'catalog_category') IS NULL")
    a("    EXEC sys.sp_executesql N'ALTER TABLE dbo.work_definition ADD catalog_category nvarchar(40) NULL;';")
    a("  IF COL_LENGTH(N'dbo.work_definition',N'interval_km') IS NULL")
    a("    EXEC sys.sp_executesql N'ALTER TABLE dbo.work_definition ADD interval_km int NULL;';")
    a("  IF COL_LENGTH(N'dbo.work_definition',N'interval_months') IS NULL")
    a("    EXEC sys.sp_executesql N'ALTER TABLE dbo.work_definition ADD interval_months int NULL;';")
    a("")
    ...
    a("  EXEC sys.sp_executesql N'UPDATE work SET name=seed.display_name, category=seed.work_category, catalog_category=seed.catalog_category, interval_km=seed.interval_km, interval_months=seed.interval_months FROM dbo.work_definition work JOIN #WorkCatalog seed ON seed.code=work.code;';")
    a("  EXEC sys.sp_executesql N'ALTER TABLE dbo.work_definition ALTER COLUMN catalog_category nvarchar(40) NOT NULL;';")
    a("")
    a("  IF COL_LENGTH(N'dbo.problem',N'category') IS NULL")
    a("    EXEC sys.sp_executesql N'ALTER TABLE dbo.problem ADD category nvarchar(32) NULL;';")
    a("  EXEC sys.sp_executesql N'UPDATE dbo.problem SET category=N''OTHER'' WHERE category IS NULL;';")
    a("  EXEC sys.sp_executesql N'ALTER TABLE dbo.problem ALTER COLUMN category nvarchar(32) NOT NULL;';")
    a("")
    ...
    a("    EXEC sys.sp_executesql N'ALTER TABLE dbo.problem DROP COLUMN suggested_repair_id;';")
    a("  END;")
    a("  IF COL_LENGTH(N'dbo.problem',N'estimated_cost') IS NOT NULL")
    a("    EXEC sys.sp_executesql N'ALTER TABLE dbo.problem DROP COLUMN estimated_cost;';")
    a("")
    ...
    a("")
    a("  DECLARE @InvalidMaintenanceIntervals bigint=0;")
    a("  EXEC sys.sp_executesql N'SELECT @Count=COUNT_BIG(*) FROM dbo.work_definition WHERE category=N''MAINTENANCE'' AND interval_km IS NULL AND interval_months IS NULL;', N'@Count bigint OUTPUT', @Count=@InvalidMaintenanceIntervals OUTPUT;")
    a("  IF @InvalidMaintenanceIntervals > 0")
    a("    THROW 51212, 'Maintenance rad bez intervala.', 1;")
    a("  DECLARE @InvalidRepairIntervals bigint=0;")
    a("  EXEC sys.sp_executesql N'SELECT @Count=COUNT_BIG(*) FROM dbo.work_definition WHERE category=N''REPAIR'' AND (interval_km IS NOT NULL OR interval_months IS NOT NULL);', N'@Count bigint OUTPUT', @Count=@InvalidRepairIntervals OUTPUT;")
    a("  IF @InvalidRepairIntervals > 0")
    ...
    a("EXEC sys.sp_executesql " + n(final_select) + ";")
